...
export class TypingsGenerator {
  ...
  public async GenerateTypings() {
    ...
    if (this._generateTypingsRequest.publisherPrefix !== undefined) {
      ...
      const publisherPrefixEntities =
        await this.getEntitiesWithPublisherPrefix();
      if (publisherPrefixEntities != undefined) {
        for (const entity of publisherPrefixEntities) {
          if (
            !this._generateTypingsRequest.entityNames.some(
              (e) => e === entity.LogicalName
            )
          ) {
    ...
    this._generateTypingsRequest.entityNames.forEach(
      async (entityLogicalName) => {
        ...
        try {
          await promises.access(
            `./${this._generateTypingsRequest.outputDirectory}`,
            constants.F_OK
          );
        } catch (error) {
        ...
        console.log(
          `Writing File /${this._generateTypingsRequest.outputDirectory}/${camelizedEntityName}.d.ts`
        );
        await promises.writeFile(
          `./${this._generateTypingsRequest.outputDirectory}/${camelizedEntityName}.d.ts`,
          typing
        );
      }
    );
  ...
  private async getEntityTyping(entityLogicalName: string) {
    ...
    const refPath = [
      dom.create.tripleSlashReferencePathDirective(
        "../node_modules/@types/xrm/index.d.ts"
      ),
    ];
    ...
    interfaceEventContext.members.push(
      dom.create.method(typingMethod, [], typeEntity)
    );
    ...
    for (const picklistColumn of pickistColumns) {
      const attrEnum = await this.parseOptionSetsAsEnums(
        entityLogicalName,
        picklistColumn.LogicalName
      );
      if (attrEnum) {
    ...
    for (const column of multiSelectColumns) {
      const attrEnum = await this.parseMultiSelectOptionSetsAsEnums(
        entityLogicalName,
        column.LogicalName
      );
      if (attrEnum) {
    ...
    const stateCodeColumns = attributes
      ...
      .filter(
        (a) =>
          (a.AttributeOf === "statecode" || a.AttributeOf === "statuscode") &&
          !a.LogicalName.endsWith("_base")
      );
    ...
    return Promise.resolve(
      dom.emit(nsEnum, emitOptions).concat(dom.emit(nsXrm))
    );
  }
   ...
   * @returns
   */
  private async getEntityAttributes(
    entityLogicalName: string
  ): Promise<IAttributeDefinition[]> {
    const respData = await this.requestData<IAttributeMetadata>(
  ...
  private async parseOptionSetsAsEnums(
    entityLogicalName: string,
    attrLogicalName: string
  ): Promise<dom.EnumDeclaration | undefined> {
    const optionset = await this.getOptionsetForAttribute(
      entityLogicalName,
      attrLogicalName
    );
   ...
   */
  private async getOptionsetForAttribute(
    entityLogicalName: string,
    attrLogicalName: string
  ): Promise<IOptionSet> {
    if (attrLogicalName === "statecode") {
      return await this.innerGetOptionsetForAttribute(
        entityLogicalName,
        attrLogicalName,
  ...
  private async innerGetOptionsetForAttribute(
    entityLogicalName: string,
    attrLogicalName: string,
    metadataType: string
  ): Promise<IOptionSet> {
    const url = `EntityDefinitions(LogicalName='${entityLogicalName}')/Attributes(LogicalName='${attrLogicalName}')/${metadataType}?$select=LogicalName&$expand=OptionSet($select=Options),GlobalOptionSet($select=Options)`;
  ...
  private async parseMultiSelectOptionSetsAsEnums(
    entityLogicalName: string,
    attrLogicalName: string
  ): Promise<dom.EnumDeclaration | undefined> {
    const optionset = await this.getMultiSelectOptionsetForAttribute(
      entityLogicalName,
      attrLogicalName
    );
    if (optionset && optionset.Options) {
   ...
   * @returns The optionset for the attribute.
   */
  private async getMultiSelectOptionsetForAttribute(
    entityLogicalName: string,
    attrLogicalName: string
  ): Promise<IOptionSet> {
    return await this.innerGetOptionsetForAttribute(
  ...
  private createOptionSetInterface(
    ...
    const enumRref = dom.create.typeParameter(
      `${pascalize(entityName)}Enum.${attr.LogicalName}`
    );
    const returnTypeParameterReference = dom.create.namedTypeReference(
      `${enumRref.name} | null`
    );

    // ADd the .getValue() method
    const choiceInterface = dom.create.interface(`${attr.LogicalName}_Options`);
    choiceInterface.members.push(
      dom.create.method(
        "getValue",
        [],
        dom.create.namedTypeReference(returnTypeParameterReference.name)
      )
    );

    // Add the .setValue() method
    const valueParameter = dom.create.parameter(
      "value",
      returnTypeParameterReference
    );
    choiceInterface.members.push(
      dom.create.method("setValue", [valueParameter], dom.type.void)
    );
  ...
  ): dom.EnumDeclaration | undefined {
    const e = dom.create.enum(
      attrLogicalName,
      true,
  ...
  private async getEntitiesWithPublisherPrefix() {
    const url = `EntityDefinitions?$select=LogicalName`;

    type EntitySchemaName = Pick<
      IEntityDefinition,
    ...
    return respData?.value.filter((e) =>
      e.LogicalName.startsWith(this._generateTypingsRequest.publisherPrefix)
    );
  }
