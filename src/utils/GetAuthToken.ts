...
export async function GenerateAuthToken(
  ...
  const authCodeUrlParameters = {
    ...
    PromptValue: PromptValue.SELECT_ACCOUNT, // Force the user to select an account each time
  };
  ...
  const authorizationCode = await authCodePromise(
    generateAuthRequest.localhostPort,
  );
 ...
 */
const authCodePromise = (port: number) =>
  new Promise<string>((resolve, reject) => {
    ...
    server = app.listen(port, () =>
      console.log(
        `Authorization code redirect server listening on port ${port}`,
      ),
    );
  });
