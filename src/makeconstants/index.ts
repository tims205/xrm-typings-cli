...
import { camelize } from "../utils/ExtensionMethods.js";
...
export class ConstantRecordsGenerator {
  ...
  public async GenerateRecordConstants() {
    ...
    for (const entity of this._constantRecordsRequest.entityNames) {
      ...
      const nsXrm = dom.create.namespace(
        `${this._constantRecordsRequest.namespace}`
      );

      const recordsEnum = dom.create.enum(
        `${camelizedEntityName}_Constants`,
        true,
      ...
      try {
        await promises.access(
          `./${this._constantRecordsRequest.outputDirectory}`,
          constants.F_OK
        );
      ...
      console.log(
        `Writing File /${this._constantRecordsRequest.outputDirectory}/${camelizedEntityName}_constants.d.ts`
      );
      await promises.writeFile(
        `./${this._constantRecordsRequest.outputDirectory}/${camelizedEntityName}_constants.d.ts`,
        dom.emit(nsXrm)
      );
    }
  }
  ...
  > {
    ...
    try {
      const getEntityResponse = await fetch(
        `${this._constantRecordsRequest.orgUrl}/api/data/v9.2/EntityDefinitions(LogicalName='${entitySchemaName.toLowerCase().trim()}')?$select=LogicalName,PrimaryNameAttribute,PrimaryIdAttribute,EntitySetName`,
        {
          //   ?$filter=LogicalName eq '${entitySchemaName.toLowerCase().trim()}?$select=LogicalName,PrimaryNameAttribute,PrimaryIdAttribute'`, {
          method: "GET",
          headers: {
      ...
      if (getEntityResponse.ok) {
        const entityData = (await getEntityResponse.json()) as Pick<
          IEntityDefinition,
          | "LogicalName"
          | "EntitySetName"
          | "PrimaryIdAttribute"
  ...
  private async GetAllRecords(
    ...
    try {
      const getRecordsResponse = await fetch(
        `${this._constantRecordsRequest.orgUrl}/api/data/v9.2/${entityDefinition.EntitySetName}?$select=${entityDefinition.PrimaryIdAttribute},${entityDefinition.PrimaryNameAttribute}&$orderby=${entityDefinition.PrimaryNameAttribute}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      if (getRecordsResponse.ok) {
        const response =
          (await getRecordsResponse.json()) as RetrieveMultipleResponse;
